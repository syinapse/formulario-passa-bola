...
class EventForm(FlaskForm):
    title = StringField(label="Título do Evento *", validators=[Length(min=5, max=30)])
    address = StringField(label="Endereço *", validators=[Length(max=50)])
    state = SelectField(label="Estado *", choices=states, validators=[])
    event_date_start = DateField(label="Inicio do Evento *", validators=[])
    event_date_end = DateField(label="Final do Evento *", validators=[])
    event_description = TextAreaField(label="Descrição do Evento *", validators=[])
    event_reward = TextAreaField(label='Descrição da Premiação *', validators=[])
    min_age = StringField(label="Idade Mínima", validators=[Length(max=2)])
    max_age = StringField(label="Idade Máxima", validators=[Length(max=2)])
    max_total_uni = StringField(label="Total Individual", validators=[Length(max=5)])
    max_total_team = StringField(label="Total de Time", validators=[Length(max=5)])
    cost_uni = StringField(label="Taxa Individual: *", validators=[Length(max=3)])
    cost_team = StringField(label="Taxa por Time: *", validators=[])
